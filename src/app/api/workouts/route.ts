...
export async function POST(request: Request) {
  ...
  const { data: plan } = await supabase
    .from("exercise_plans")
    .select(
      "id, plan_exercises(id, name, sets, reps, weight_lb, rest_seconds, sort_order)",
    )
  ...
  const { data: session, error } = await supabase
    ...
    .insert({ profile_id: user.id, exercise_plan_id: plan.id })
    .select("id")
    .single();
  if (error || !session)
    return NextResponse.json(
  ...
  if (exercisesError || !sessionExercises)
    return NextResponse.json(
      { error: "Could not prepare workout." },
      { status: 500 },
    );
  return NextResponse.json(
    {
