    ...
    <aside className="flex w-full shrink-0 flex-col border-b border-slate-200 bg-white lg:min-h-screen lg:w-64 lg:border-r lg:border-b-0">
      ...
      >
        {links.map((link) => {
          ...
          return (
            <Link
              className={`flex shrink-0 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? "bg-emerald-50 text-emerald-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}
              href={link.href}
              key={link.href}
            >
              <span
                aria-hidden="true"
                className="grid h-5 w-5 place-items-center text-base"
              >
                {link.icon}
              </span>
              {link.label}
