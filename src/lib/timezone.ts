...
export function dateKeyInTimeZone(value: Date | string = new Date(), timeZone = browserTimeZone()) {
  return keyFromParts(partsInTimeZone(typeof value === "string" ? new Date(value) : value, timeZone));
}

/** Returns the UTC instant at midnight for a calendar date in the supplied zone. */
export function startOfDayInTimeZone(value: Date | string = new Date(), timeZone = browserTimeZone()) {
...
export function formatInTimeZone(value: Date | string, options: Intl.DateTimeFormatOptions, timeZone = browserTimeZone()) {
  return new Intl.DateTimeFormat("en", { ...options, timeZone }).format(typeof value === "string" ? new Date(value) : value);
}
